...
def run_simulation(graph, vehicle_configs, heuristic_name, solver_params):
    ...
    data = {
        ...
        "lookahead_horizon": solver_params.get("lookahead_horizon", 20),
        "max_steps": 1000
    }
    ...
    try:
        ...
        # We redirect stderr=sys.stderr so C++ output goes directly to the terminal,
        # bypassing Python's internal capture mechanism.
        ...
        # Use communicate to send input and wait for the process to finish.
        # This will block the Streamlit thread until the simulation is done, 
        # but the C++ output will be visible instantly in the terminal.
        output_data, _ = process.communicate(input=input_str, timeout=86400)
        
        runtime = time.perf_counter() - start_time
        
        if process.returncode != 0:
            # If the process fails, the error log (if any) was already streamed to sys.stderr
            return [], f"Solver terminated unexpectedly (RC={process.returncode}). Check terminal output for log.", runtime

        if not output_data.strip():
             return [], "No output (JSON) received from solver. Check terminal log for errors.", runtime

        history = json.loads(output_data)
        return history, None, runtime

    except subprocess.TimeoutExpired:
        # Note: If the process times out, kill it safely
        process.kill()
        return [], "Solver timed out after 86400 seconds.", 0.0
...
with st.sidebar:
    ...
    uploaded_run = st.file_uploader("Upload Run JSON", type="json", key="load_run")
...
if st.session_state.sim_data:
    ...
    if not hist:
        ...
        st.dataframe(stats['vehicle_stats'], hide_index=True, use_container_width=True)
        ...
        if not service_logs.empty:
            st.dataframe(service_logs, hide_index=True, use_container_width=True)
        else:
        ...
        if not resupply_df.empty:
            st.dataframe(resupply_df, hide_index=True, use_container_width=True)
        else:
